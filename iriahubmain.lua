...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://117394884545382"
    ...
    if not success then
        warn("[iria Hub] Error notification")
    end
end

NotificacaoNightMystic("iria Hub", "script has loaded successfully!")

-- ========================================
-- SAVE SYSTEM (Optimized)
-- ========================================
local HttpService = Services.HttpService
local FolderName = "iria Hub"
...
QuestNeta = function()
	...
	local Library
	local _uiUrl = "https://pastefy.app/YGq0OCsv/raw"
	for _attempt = 1, 5 do
		...
		if _ok and _result then
			...
			warn("[iria Hub] UI load attempt " .. _attempt .. " failed, retrying...")
			task.wait(2)
		end
	end
	if not Library then
		error("[iriaHub] Failed to load UI after 5 attempts. Check your internet or executor.")
	end
	Library = Library:MakeWindow({
    Title = "iria Hub| Blox fruit",
    SubTitle = "by iria",
    SaveFolder = "iria Hub"
})
-- Criar ScreenGui
...
imageButton.Image = "rbxassetid://93161054586708"
...
local Discord = Library:MakeTab({
    Title = "Discord",
    Icon = "rbxassetid://117394884545382"
})
...
Discord:AddDiscordInvite({
    ...
    Logo = "rbxassetid://115119838430718",
    Invite = "https://discord.gg/rReAGm43Pn"
})
local credits = Discord:AddParagraph({
    Title = "Credits For Someone Peoples",
    Desc = ""
})
credits:SetDesc("ai hỏi")
...
NotificacaoNightMystic("iria Hub", "✅All systems have been downloaded!")
